...
<%@ page
	import="com.zoho.service.impl.*, com.zoho.service.*,com.zoho.beans.*,java.util.*,javax.servlet.ServletOutputStream,java.io.*"%>
<!DOCTYPE html>
<html>
<head>
		...
		<table class="table table-hover">
			...
			<tbody
				...
				for (CartBean item : cartItems) {
					...
					//dis1 = product.getPrice()- (product.getPrice() * product.getDiscount()/ 100);
				...
				<tr>
					...
					<td><form method="post" action="./UpdateToCart">
							<input type="number" name="pqty" value="<%=prodQuantity%>"
								...
								style="max-width: 80px;"></form></td>
					<td><a
						href="cartDetails.jsp?add=1&uid=<%=userName%>&pid=<%=product.getProdId()%>&avail=<%=product.getStock()%>&qty=<%=prodQuantity%>"><i
							class="fa fa-plus"></i></a></td>
					<td><a
						href="cartDetails.jsp?add=0&uid=<%=userName%>&pid=<%=product.getProdId()%>&avail=<%=product.getStock()%>&qty=<%=prodQuantity%>"><i
				...
				<%if(totAmount >=1000){
					...
					user.setCredit(user.getCredit()+(int)totAmount/50);
					System.out.println(user.getCredit());
				}%>
				...
				<tr style="background-color: grey; color: white;">
					<td colspan="6" style="text-align: center;">Credit</td>
